...
class EngineClient(private val path: String = SOCKET_PATH) {
     ...
     * (BRIDGE.md, «Файлы списков и движок»): до её появления сервер отвечает unknown-command, и
     * это превращается в EngineError — логика показывает человеку, что списки на движок не
     * легли, вместо того чтобы молча собрать спеку со ссылками на несуществующие файлы.
